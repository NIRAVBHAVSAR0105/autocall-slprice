...
// ─── Personalized ExoML endpoint ─────────────────────
// Exotel fetches this — we serve personalized script per customer
app.get('/exoml', (req, res) => {
  ...
  // Try to get script by CustomField (callId) first, then by phone
  const log = callLog[customField];
  const script = log?.script || scriptStore[callFrom] || buildTemplate({ name: 'aap', outstanding: 'outstanding' });

  console.log('Serving personalized script for:', callFrom, '|', script.substring(0, 50));
...
app.post('/api/call', async (req, res) => {
  ...
  try {
    ...
    console.log('Call initiated! SID:', callLog[callId].exotelSid);
    res.json({ success: true, callSid: callLog[callId].exotelSid, script });
  } catch (err) {
    console.error('Call error:', err?.response?.status, JSON.stringify(err?.response?.data));
...
app.listen(PORT, () => console.log(`AutoCall v7 Personalized ready on port ${PORT}`));

// Serve frontend directly from server — no CORS issues!
...
app.get('/', (req, res) => {
  ...
  if (fs.existsSync(htmlPath)) {
    ...
    res.send('<h2>AutoCall Server is running! Upload index.html to serve the dashboard.</h2>');
  }
});
