...
// ✅ FIXED: Mumbai cluster uses api.in.exotel.com
const EXOTEL_BASE = `https://${EXOTEL_API_KEY}:${EXOTEL_API_TOKEN}@api.in.exotel.com/v1/Accounts/${EXOTEL_ACCOUNT_SID}`;
...
app.post('/api/call', async (req, res) => {
  ...
  try {
    ...
    console.log('Script stored for', cleanPhone, '| Calling via Mumbai cluster...');
    ...
    console.error('Call error:', JSON.stringify(err?.response?.data) || err.message);
...
app.get('/health', (req, res) => res.json({
  ...
  version: 'v5-mumbai',
  provider: 'Exotel',
  cluster: 'Mumbai (api.in.exotel.com)',
...
app.listen(PORT, () => console.log(`AutoCall v5 Mumbai ready on port ${PORT}`));
